...
VOID Fini(int code, VOID * v) {
    ...
    while (i < 20) {
        ...
        const ADDRINT * k = new ADDRINT (addrL[max_rout]);
        outFile <<"Name = "<< * s <<"   Address = "<<std::hex<< * k <<"   Misses = "<<std::dec<< temp_1[max_rout] << endl;
   ...
   int j = 0;
    while (j < 20) {
        ...
        const ADDRINT * k = new ADDRINT (addrL[max_rout]);
        outFile <<"Name = "<< * s <<"   Address = "<<std::hex<< * k <<"   Misses = "<<std::dec<< temp_2[max_rout] << endl;
        temp_2[max_rout] = 0;
        j++;
    }
    outFile <<
